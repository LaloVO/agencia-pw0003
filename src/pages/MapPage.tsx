...
const MapPage = () => {
  ...
  const fallbackToken = 'pk.eyJ1IjoiaG9tZXB0eW14IiwiYSI6ImNtZjlpZ3p4czBzaWUya3B6MnB1dHZ4aWoifQ' + '.' + 'ZKWLoVLu-fVaTXRD7HfXTg';
  const mapboxToken = (
  ...
  const filtered = useMemo(() => {
    return properties.filter((p) => {
      ...
      if (queryParam) {
        ...
        const colonyMatch = (p.colonia || '').toLowerCase().includes(q);
        const dirMatch = (p.direccion || '').toLowerCase().includes(q);
        if (!nameMatch && !descMatch && !colonyMatch && !dirMatch) return false;
      }
      
      // URL action filter: id_tipo_accion: 1 = Venta (Comprar), 2 = Renta
      if (actionParam) {
        if (actionParam === 'comprar' && p.id_tipo_accion !== 1) return false;
        if (actionParam === 'rentar' && p.id_tipo_accion !== 2) return false;
      }
  ...
  }, [properties, filters, queryParam, actionParam]);
  ...
  return (
    <>
      ...
      <main className="pt-24 md:pt-28 h-screen flex overflow-hidden">
        {/* Map column */}
